...
export function RegisterPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({ defaultValues: { base_currency: 'NGN' } })
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} placeholder="Minimum 8 characters" className={`${inp} pl-10 pr-10`} {...register('password', { required: true, minLength: 8 })} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
              {errors.password && <p className="text-xs text-red-500">Minimum 8 characters required</p>}
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">Base currency</label>
              <div className="relative">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                <select className={`${inp} pl-10 pr-4`} {...register('base_currency')}>
                  {['NGN','USD','EUR','GBP','USDT'].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <p className="text-xs text-gray-400">All amounts converted to this for reporting</p>
            </div>
            ...
            <p className="text-center text-sm text-gray-500">
              ...
              <Link to="/login" className="font-semibold hover:underline" style={{ color: 'var(--pl-primary)' }}>Sign in</Link>
            </p>
          </form>
        </div>
      </div>
